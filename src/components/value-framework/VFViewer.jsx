import { VF_DIMENSIONS } from "@/mock/value-framework";
...
function EqCard({ dim, onClick, compact, style }) {
  const [hov, setHov] = useVState(false);
  const c = dim.color;
  return (
    <div
      onClick={() => onClick(dim)}
...
function VFLevel1({ onDimSelect, onNavigateToIchom }) {
  const [mode, setMode] = useVState("equation"); // "equation" | "grid"
  const [ichomModal, setIchomModal] = useVState(null);
...
function ValueFrameworkViewer({ onNavigateToIchom }) {
  const [level, setLevel] = useVState(1); // 1 | 2 | 3
  const [activeDim, setActiveDim] = useVState(null);
  const [activeMeasure, setActiveMeasure] = useVState(null);
